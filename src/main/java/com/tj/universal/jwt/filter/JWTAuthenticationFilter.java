package com.tj.universal.jwt.filter;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.tj.universal.entity.ResultDto;
import com.tj.universal.jwt.config.SpringSecurityConfig;
...
public class JWTAuthenticationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (StringUtils.isBlank(header) || !header.startsWith(JwtUtil.TOKEN_PREFIX)) {
            ...
            response.getWriter().write(JSON.toJSONString(ResultDto.fail("Token为空")));
            return;
        }
        try {
            ...
            response.setCharacterEncoding("UTF-8");
            response.getWriter().write(JSON.toJSONString(ResultDto.fail("Token已过期",401)));
